...
describe('DataDog', () => {
  ...
  describe('incrementMetric', () => {
    it('behaves as a convenience method for sendMetric with quanity 1', async done => {
      const datadog = new DataDog(dogapiMock, 'e2e-namespace')
      const metricName = 'step_error'
      const sendMetricMock = jest.fn()
      datadog.sendMetric = sendMetricMock
      await datadog.incrementMetric(metricName)
      expect(sendMetricMock).toHaveBeenCalledWith(metricName, 1)
      sendMock.mockClear()
      done()
    })
  })
